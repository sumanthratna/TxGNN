...
class TxGNN:
    def __init__(
        ...
        self.G = data.G
        self.df, self.df_train, self.df_valid, self.df_test = (
            data.df,
            data.df_train,
            data.df_valid,
            data.df_test,
    ...
    def finetune(
                    ...
                    temp_d.update(
                        {
                            ...
                            "Validation Relation Performance": self.wandb.Table(
                                data=to_wandb_table(auroc_rel, auprc_rel),
        ...
        if self.weight_bias_track:
            ...
            temp_d.update(
                {
                    "Testing Loss": loss,
                    "Testing Relation Performance": self.wandb.Table(
                        data=to_wandb_table(auroc_rel, auprc_rel),
                        columns=["rel_id", "Rel", "AUROC", "AUPRC"],
                    ),
                }
    ...
    def predict(self, df):
        ...
        for etype in g.canonical_etypes:
            ...
            dst = (
                torch.Tensor(df_temp.y_idx.values).to(self.device).to(dtype=torch.int64)
            )
    ...
    def retrieve_sim_diseases(self, relation, k=5, path=None):
        ...
        sim = sim_all_etypes[etype][
            np.array(
                [
                    diseaseid2id_etypes[etype][i.item()]
    ...
    def train_graphmask(
        ...
        for layer in reversed(list(range(self.graphmask_model.count_layers()))):
            self.graphmask_model.enable_layer(
                layer
            )  ## enable baselines and gates parameters
            ...
            for epoch in range(epochs_per_layer):
                ...
                (
                    updated_predictions_pos,
                    updated_predictions_neg,
                    penalty,
                    num_masked,
                ) = self.graphmask_model.graphmask_forward(
                        self.G,
                ...
                loss_pred = F.binary_cross_entropy(
                    updated_predictions, torch.Tensor(labels).float().to(self.device)
                ).item()

                original_predictions = original_predictions.to(self.device)
                loss_pred_ori = F.binary_cross_entropy(
                    original_predictions, torch.Tensor(labels).float().to(self.device)
                ).item()
                ...
                if self.weight_bias_track == "True":
                    self.wandb.log(
                        {
                            "divergence": g_moving_average.get_value(),
    ...
    def load_pretrained_graphmask(
        ...
        state_dict = torch.load(
            os.path.join(path, "graphmask_model.pt"), map_location=torch.device("cpu")
        )
        if next(iter(state_dict))[:7] == "module.":
            # the pretrained model is from data-parallel module
            from collections import OrderedDict
    ...
    def retrieve_gates_scores_penalties(self, relation, no_base=False):
        ...
        whole_graph = [
            original_predictions_pos,
            original_predictions_neg,
            updated_predictions_pos,
            updated_predictions_neg,
            num_masked,
            gates,
    ...
    def retrieve_save_gates(self, path):
        ...
        for etypes in G.canonical_etypes:
            ...
            df_temp[self.relation + "_layer1_att"] = scores[0][etype].reshape(
                -1,
            )
            df_temp[self.relation + "_layer2_att"] = scores[1][etype].reshape(
                -1,
            )

            all_att_frames.append(df_temp)

        all_att_df = (
            pd.concat(all_att_frames, ignore_index=True)
            if all_att_frames
            else pd.DataFrame()
        )

...
def preprocess_kg(path, split, test_size=0.05, one_hop=False, mask_ratio=0.1):
    if split in [
        ...
        test_kg = ds.get_test_kg_for_disease(
            name2id[split], test_size=test_size, one_hop=one_hop, mask_ratio=mask_ratio
        )
        all_kg = ds.kg
        all_kg["split"] = "train"
        test_kg["split"] = "test"
        df = (
            pd.concat([all_kg, test_kg])
    ...
    for i in tqdm(unique_relation):
        if ("_" in i) and (i.split("_")[0] == i.split("_")[1]):
            ...
            df_temp["check_string"] = df_temp.apply(
                lambda row: "_".join(sorted([str(row["x_id"]), str(row["y_id"])])),
                axis=1,
            )
            undirected_index.append(
                df_temp.drop_duplicates("check_string").index.values.tolist()
            )
        else:
            # undirected
            d_off = df[df.relation == i]
            undirected_index.append(
                d_off[d_off.x_type == d_off.x_type.iloc[0]].index.values.tolist()
            )
    ...
    unique_node_types = np.unique(
        np.append(np.unique(df.x_type.values), np.unique(df.y_type.values))
    )
    ...
    for i in tqdm(unique_node_types):
        names = np.unique(
            np.append(
                df[df.x_type == i]["x_id"].values, df[df.y_type == i]["y_id"].values
            )
        )
        names2idx = dict(zip(names, list(range(len(names)))))
        df.loc[df.x_type == i, "x_idx"] = df[df.x_type == i]["x_id"].apply(
            lambda x: names2idx[x]
        )
        df.loc[df.y_type == i, "y_idx"] = df[df.y_type == i]["y_id"].apply(
            lambda x: names2idx[x]
        )
        idx_map[i] = names2idx
...
def random_fold(df, fold_seed, frac):
    ...
    df_train = (
        pd.concat(train_parts) if train_parts else pd.DataFrame(columns=df.columns)
    )
    df_valid = (
        pd.concat(valid_parts) if valid_parts else pd.DataFrame(columns=df.columns)
    )
    df_test = pd.concat(test_parts) if test_parts else pd.DataFrame(columns=df.columns)
...
def complex_disease_fold(df, fold_seed, frac):
    ...
    train, valid, test = np.split(
        unique_diseases,
        [
            int(frac[0] * len(unique_diseases)),
...
def few_edeges_to_kg_fold(df, fold_seed, frac):
    ...
    disease2num_neighbors_1 = dict(
        df_not_dd[df_not_dd.x_type == "disease"].groupby("x_idx").y_id.agg(len)
    )
    disease2num_neighbors_2 = dict(
        df_not_dd[df_not_dd.y_type == "disease"].groupby("y_idx").x_id.agg(len)
    )
    ...
    for key in set(disease2num_neighbors_1).union(disease2num_neighbors_2):
        disease2num_neighbors[key] = disease2num_neighbors_1.get(
            key, 0
        ) + disease2num_neighbors_2.get(key, 0)

    disease_with_less_than_3_connections_in_kg = np.array(
        [i for i, j in disease2num_neighbors.items() if j <= 3]
    )
    unique_diseases = df_dd.y_idx.unique()
    train_val_diseases = np.setdiff1d(
        unique_diseases, disease_with_less_than_3_connections_in_kg
    )
...
def few_edeges_to_indications_fold(df, fold_seed, frac):
    ...
    disease2num_indications = dict(
        df_dd[(df_dd.y_type == "disease") & (df_dd.relation == "indication")]
        .groupby("x_idx")
        .y_id.agg(len)
    )

    disease_with_less_than_3_indications_in_kg = np.array(
        [i for i, j in disease2num_indications.items() if j <= 3]
    )
    unique_diseases = df_dd.y_idx.unique()
    train_val_diseases = np.setdiff1d(
        unique_diseases, disease_with_less_than_3_indications_in_kg
    )
...
def create_fold_cv(df, split_num, num_splits):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(unique_diseases)):
        train_index, valid_index, _ = np.split(
            train_index, [int(0.9 * len(train_index)), int(len(train_index))]
        )
        split_num_idx[i + 1] = {
            "train": unique_diseases[train_index],
            "valid": unique_diseases[valid_index],
            "test": unique_diseases[test_index],
        }

    train, valid, test = (
        split_num_idx[split_num]["train"],
        split_num_idx[split_num]["valid"],
...
def create_fold(
    ...
    if method == "random":
        ...
        out["test"] = out[
            "valid"
        ]  # this is to avoid error but we are not using testing set metric here
    else:
        # disease split
...
def create_split(df, split, disease_eval_index, split_data_path, seed):
    ...
    if split == "complex_disease_cv":
        if seed < 1 or seed > 20:
            raise ValueError(
                "Complex disease cross validation 20 folds, select seed from 1-20."
            )
        df_train, df_valid, df_test = create_fold_cv(df, split_num=seed, num_splits=20)
    else:
        df_train, df_valid, df_test = create_fold(
            df,
            fold_seed=seed,
...
def construct_negative_graph_each_etype(graph, k, etype, method, weights, device):
    ...
    if method == "corrupt_dst":
        ...
        (method == "multinomial_src")
        or (method == "inverse_src")
        or (method == "fix_src")
    ):
        neg_dst = dst.repeat_interleave(k)
        try:
...
class Full_Graph_NegSampler:
    ...
    def __call__(self, graph):
        ...
        return dgl.heterograph(
            out,
            num_nodes_dict={
                ntype: graph.number_of_nodes(ntype) for ntype in graph.ntypes
            },
        )
...
def get_all_metrics_fb(
    pred_score_pos, pred_score_neg, scores, labels, G, full_mode=False
):
...
def evaluate_fb(
    model, g_pos, g_neg, G, dd_etypes, device, return_embed=False, mode="valid"
):
    model.eval()
    pred_score_pos, pred_score_neg, pos_score, neg_score = model(
        G, g_neg, g_pos, pretrain_mode=False, mode=mode
    )
    ...
    scores = torch.sigmoid(
        torch.cat((pos_score, neg_score)).reshape(
            -1,
        )
    )
...
def evaluate_gnnexplainer(
    ...
    with torch.no_grad():
        ...
        original_predictions = torch.sigmoid(torch.cat((pos_score, neg_score))).to(
            "cpu"
        )

        updated_predictions_pos, updated_predictions_neg, penalty, num_masked = (
            model.gnnexplainer_forward(
                G,
                g_valid_pos,
                g_valid_neg,
        ...
        loss_pred = F.binary_cross_entropy(
            updated_predictions, torch.Tensor(labels).float().to(device)
        ).item()

        original_predictions = original_predictions.to(device)
        loss_pred_ori = F.binary_cross_entropy(
            original_predictions, torch.Tensor(labels).float().to(device)
        ).item()
...
def evaluate_graphmask(
    ...
    with torch.no_grad():
        ...
        if weight_bias_track:
            wandb.log(
                {
                    mode + " divergence": float(loss.mean().item()),
...
def evaluate_ib(
    ...
    with torch.no_grad():
        ...
        original_predictions_pos, original_predictions_neg, _, _ = model.ib_forward(
            G,
            g_valid_pos,
            g_valid_neg,
            graphmask_mode=False,
...
def evaluate_mb(
    ...
    model.eval()
    # model = model.to('cpu')
    pred_score_pos, pred_score_neg, pos_score, neg_score = model.forward_minibatch(
        g_pos.to(device),
        g_neg.to(device),
        [G.to(device), G.to(device)],
        G.to(device),
        mode=mode,
        pretrain_mode=False,
    )
...
def process_df(df_train, edge_dict):
    ...
    df_train = df_train[
        ["x_type", "x_idx", "relation_idx", "y_type", "y_idx", "degree", "label"]
    ].rename(
        columns={
            "x_type": "head_type",
            "x_idx": "head",
...
def reverse_rel_generation(df, df_valid, unique_rel):
    ...
    return pd.concat(all_parts, ignore_index=True).reset_index(drop=True)


def get_wandb_log_dict(
    auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc, mode
):
    ...
    name_mapping = {
        ("drug", "contraindication", "disease"): " Contraindication ",
...
def obtain_protein_random_walk_profile(
    disease, num_walks, path_len, g, disease_etypes, disease_nodes, walk_mode
):
    random_walks = []
...
def process_disease_area_split(data_folder, df, df_test, split):
    ...
    id2idx = dict(
        df[df.x_type == "disease"][["x_id", "x_idx"]].drop_duplicates().values
    )
    id2idx.update(
        dict(df[df.y_type == "disease"][["y_id", "y_idx"]].drop_duplicates().values)
    )
    ...
    disease_list["node_idx"] = disease_list.node_id.apply(
        lambda x: map_node_id_2_idx(x, id2idx)
    )
...
def create_dgl_graph(df_train, df):
    ...
    g = dgl.heterograph(
        DGL_input, num_nodes_dict={i: int(output[i]) + 1 for i in output.keys()}
    )
    ...
    for etype in g.etypes:
        ...
        g.edges[etype].data["id"] = (
            torch.ones(g.number_of_edges(etype), dtype=torch.long) * edge_dict[etype]
        )
...
def disease_centric_evaluation(
    df,
    df_train,
    df_valid,
    df_test,
    ...
    idx2id_drug = dict(
        df[df.x_type == "drug"][["x_idx", "x_id"]].drop_duplicates().values
    )
    idx2id_drug.update(
        dict(df[df.y_type == "drug"][["y_idx", "y_id"]].drop_duplicates().values)
    )

    idx2id_disease = dict(
        df[df.x_type == "disease"][["x_idx", "x_id"]].drop_duplicates().values
    )
    idx2id_disease.update(
        dict(df[df.y_type == "disease"][["y_idx", "y_id"]].drop_duplicates().values)
    )
    ...
    id2name_disease = dict(
        df_[df_.x_type == "disease"][["x_id", "x_name"]].drop_duplicates().values
    )
    id2name_disease.update(
        dict(df_[df_.y_type == "disease"][["y_id", "y_name"]].drop_duplicates().values)
    )

    id2name_drug = dict(
        df_[df_.x_type == "drug"][["x_id", "x_name"]].drop_duplicates().values
    )
    id2name_drug.update(
        dict(df_[df_.y_type == "drug"][["y_id", "y_name"]].drop_duplicates().values)
    )
    ...
    def calculate_metrics(rel, preds_all, labels_all, mode="drug", subset_mode=True):
        ...
        for entity_id in ids_all:
            ...
            for i, j in ranked_list_k.items():
                ...
                in_ranked_list[i][entity_id] = [
                    id2name[idx2id[x]] for x in recalled_list
                ]
                not_in_ranked_list[i][entity_id] = [
                    id2name[idx2id[x]] for x in pos_idx if x not in recalled_list
                ]
                if len(pos_idx) == 0:
                    ...
                    if simulate_random:
                        Recall_Random[i][entity_id] = np.mean(
                            [
                                len(np.intersect1d(sim_trial[: k_num[i]], pos_idx))
                                / len(pos_idx)
                                for sim_trial in ranked_list_random
                            ]
                        )
                    else:
                        Recall_Random[i][entity_id] = k_num[i] / num_items[rel]

                    Enrichment[i][entity_id] = len(recalled_list) / (
                        Recall_Random[i][entity_id] * len(pos_idx)
                    )
    ...
    def summary(result, rel_type, mode="drug", show_plot=True, verbose=True):
        ...
        if show_plot:
            ...
            preds_ = np.concatenate(
                [np.array(list(j.values())) for i, j in results["Prediction"].items()]
            ).reshape(
                -1,
            )
            labels_ = np.concatenate(
                [np.array(list(j.values())) for i, j in results["Labels"].items()]
            ).reshape(
                -1,
            )
    ...
    def get_scores_disease(rel, disease_ids):
        ...
        for disease_id in tqdm(disease_ids):
            ...
            out = {}
            src = (
                torch.Tensor([disease_id] * len(labels))
                .to(device)
            ...
            pred = (
                pred_score_rel[("disease", rel, "drug")]
                .reshape(
                    -1,
                )
                .detach()
            ...
            lab = {
                idx2id_drug[i]: labels[i]
                for i in g_eval.edges()[1].detach().cpu().numpy()
            }
            preds_contra[idx2id_disease[disease_id]] = {
                idx2id_drug[i]: pred[idx]
                for idx, i in enumerate(g_eval.edges()[1].detach().cpu().numpy())
            }
            labels_contra[idx2id_disease[disease_id]] = lab
            ids_contra[idx2id_disease[disease_id]] = (
                g_eval.edges()[1].detach().cpu().numpy()
            )
        ...
        return (
            ...
            labels_contra,
            drug_nodes,
            [id2name_drug[idx2id_drug[i]] for i in drug_nodes],
        )
    ...
    if disease_ids is None:
        ...
        for rel_type in disease_rel_types:
            ...
            preds_, labels_, drug_idxs, drug_names = get_scores_disease(
                rel_type, disease_ids
            )
            preds_all[rel_type], labels_all[rel_type] = preds_, labels_
            results, _ = calculate_metrics(
                rel_type, preds_all, labels_all, mode="disease"
            )
            out_dict_mean, out_dict_std = summary(
                results, rel_type, mode="disease", show_plot=show_plot, verbose=verbose
            )
            org_out = [
                [idx, i, out_dict_mean[i], out_dict_std[i]]
                for idx, i in enumerate(out_dict_mean.keys())
            ]
            ...
            if weight_bias_track:
                temp_d.update(
                    {
                        "disease_centric_evaluation_" + rel_type: wandb.Table(
                            data=org_out, columns=["metric_id", "metric", "mean", "std"]
                        )
                    }
                )
        ...
        if return_raw:
            ...
            return {
                rel_type: pd.DataFrame.from_dict(metrics_all[rel_type])
                for rel_type in disease_rel_types
            }
        ...
        if return_raw:
            out = {
                "prediction": preds_all[rel_type],
                "label": labels_all[rel_type],
                "result": metrics_all[rel_type],
            }
            return out
        else:
...
def find_two_hops(x_idx_value, x_type_value, df):
    ...
    one_hop = df[
        ((df["x_idx"] == x_idx_value) & (df["x_type"] == x_type_value))
        | ((df["y_idx"] == x_idx_value) & (df["y_type"] == x_type_value))
    ]

    # Collect all unique neighbor pairs
    neighbors = set(zip(one_hop["x_idx"], one_hop["x_type"])) | set(
        zip(one_hop["y_idx"], one_hop["y_type"])
    )
    ...
    two_hop = df.merge(
        neighbors_df, left_on=["x_idx", "x_type"], right_on=["idx", "type"]
    )
    two_hop = pd.concat(
        [
            two_hop,
            df.merge(
                neighbors_df, left_on=["y_idx", "y_type"], right_on=["idx", "type"]
            ),
        ]
    )
...
def remove_random_edges(hetero_graph, K):
    ...
    for etype in hetero_graph.canonical_etypes:
        ...
        edges_to_remove = np.random.choice(
            num_existing_edges, num_edges_to_remove, replace=False
        )
        edge_ids = torch.tensor(edges_to_remove, dtype=torch.int64).to(
            hetero_graph.device
        )
...
def remove_relation_type(hetero_graph, relation_type):
    ...
    etypes_to_remove = [
        etype for etype in hetero_graph.canonical_etypes if etype[1] == relation_type
    ]
    etypes_to_remove += [
        etype
        for etype in hetero_graph.canonical_etypes
        if etype[1] == "rev_" + relation_type
    ]
...
def add_random_edges(hetero_graph, K):
    ...
    for etype in hetero_graph.canonical_etypes:
        ...
        src_tensor = torch.tensor(src_random_nodes, dtype=torch.int64).to(
            hetero_graph.device
        )
        dst_tensor = torch.tensor(dst_random_nodes, dtype=torch.int64).to(
            hetero_graph.device
        )

...
class DistMultPredictor(nn.Module):
    def __init__(
        ...
        if proto:
            ...
            if sim_measure in ["bert", "profile+bert"]:
                ...
                self.disease_dict = dict(
                    df[df.x_type == "disease"][["x_idx", "x_id"]].values
                )
                self.disease_dict.update(
                    dict(df[df.y_type == "disease"][["y_idx", "y_id"]].values)
                )
                ...
                if bert_measure == "disease_name":
                    ...
                    self.bert_embed = np.load(
                        "/n/scratch3/users/k/kh278/disease_embeds_single_def.npy"
                    )
                    df_nodes_bert = pd.read_csv(
                        "/n/scratch3/users/k/kh278/disease_nodes_for_BERT_embeds.csv"
                    )

                df_nodes_bert["node_id"] = df_nodes_bert.node_id.apply(
                    lambda x: convert2str(x)
                )
                self.id2bertindex = dict(
                    zip(df_nodes_bert.node_id.values, df_nodes_bert.index.values)
                )
            ...
            for etype in self.etypes_dd:
                ...
                if sim_measure == "all_nodes_profile":
                    ...
                    diseases_profile = {
                        i.item(): obtain_protein_random_walk_profile(
                            i,
                            num_walks,
                            path_length,
                            G,
                    ...
                    diseases_profile = {
                        i.item(): torch.Tensor(
                            self.bert_embed[
                                self.id2bertindex[self.disease_dict[i.item()]]
                            ]
                        )
                        for i in all_disease_ids
                    }
                ...
                diseaseid2id = dict(
                    zip(
                        all_disease_ids.detach().cpu().numpy(),
                        range(len(all_disease_ids)),
                    )
                )
                disease_profile_tensor = torch.stack(
                    [diseases_profile[i.item()] for i in all_disease_ids]
                )
    ...
    def apply_edges(self, edges):
        h_u = edges.src["h"]
        h_v = edges.dst["h"]
        rel_idx = self.rel2idx[edges.canonical_etype]
        h_r = self.W[rel_idx]
    ...
    def forward(self, graph, G, h, pretrain_mode, mode, block=None, only_relation=None):
        with graph.local_scope():
            ...
            if pretrain_mode:
                ...
                etypes_all = [
                    i
                    for i in graph.canonical_etypes
                    if graph.edges(etype=i)[0].shape[0] != 0
                ]
                ...
                for etype in etypes_train:
                    if self.proto:
                        ...
                        if self.sim_measure in [
                            "protein_profile",
                            "all_nodes_profile",
                            "protein_random_walk",
                            "bert",
                            "profile+bert",
                            "all_nodes_profile_more",
                        ]:
                            try:
                                sim = self.sim_all_etypes[etype][
                                    np.array(
                                        [
                                            self.diseaseid2id_etypes[etype][i.item()]
                                ...
                                for i in h_disease["disease_query_id"][0]:
                                    if (
                                        i.item()
                                        not in self.diseases_profile_etypes[etype]
                                    ):
                                        if self.sim_measure == "all_nodes_profile":
                                            ...
                                            self.sim_measure == "all_nodes_profile_more"
                                        ):
                                            self.diseases_profile_etypes[etype][
                                                i.item()
                                            ] = obtain_disease_profile(
                                ...
                                profile_query = [
                                    self.diseases_profile_etypes[etype][i.item()]
                                    for i in h_disease["disease_query_id"][0]
                                ]
                                profile_query = torch.cat(profile_query).view(
                                    len(profile_query), -1
                                )

                                profile_keys = [
                                    self.diseases_profile_etypes[etype][i.item()]
                                    for i in h_disease["disease_key_id"][0]
                                ]
                                profile_keys = torch.cat(profile_keys).view(
                                    len(profile_keys), -1
                                )
                            ...
                            if src_h.shape[0] == src_h_keys.shape[0]:
                                ...
                                embed = h_disease["disease_key"][
                                    torch.topk(sim, self.k + 1).indices[:, 1:]
                                ]
                            else:
                                ## during evaluation...
                                ...
                                coef = F.normalize(coef, p=1, dim=1)
                                embed = h_disease["disease_key"][
                                    torch.topk(sim, self.k).indices[:, :]
                                ]
                            out = torch.mul(
                                embed, coef.unsqueeze(dim=2).to(self.device)
                            ).sum(dim=1)
                        ...
                        if self.sim_measure in [
                            ...
                            if self.agg_measure == "learn":
                                coef_all = self.m(
                                    self.W_gate["disease"](
                                        torch.cat(
                                            (h_disease["disease_query"], out), dim=1
                                        )
                                    )
                                )
                                proto_emb = (1 - coef_all) * h_disease[
                                    "disease_query"
                                ] + coef_all * out
                                ...
                                if src == "disease":
                                    coef_all = exponential(
                                        G.out_degrees(etype=etype)[
                                            torch.where(
                                                graph.out_degrees(etype=etype) != 0
                                            )
                                        ],
                                        self.exp_lambda,
                                    ).reshape(-1, 1)
                                    ...
                                    coef_all = exponential(
                                        G.in_degrees(etype=etype)[
                                            torch.where(
                                                graph.in_degrees(etype=etype) != 0
                                            )
                                        ],
                                        self.exp_lambda,
                                    ).reshape(-1, 1)
                            ...
                            if self.agg_measure == "learn":
                                coef_src = self.m(
                                    self.W_gate[src](
                                        torch.cat((src_h, sim_emb_src), dim=1)
                                    )
                                )
                                coef_dst = self.m(
                                    self.W_gate[dst](
                                        torch.cat((dst_h, sim_emb_dst), dim=1)
                                    )
                                )
                            elif self.agg_measure == "rarity":
                                ...
                                coef_dst = exponential(
                                    G.in_degrees(etype=etype)[
                                        torch.where(graph.in_degrees(etype=etype) != 0)
                                    ],
                                    self.exp_lambda,
                                ).reshape(-1, 1)
                            elif self.agg_measure == "heuristics-0.8":
                            ...
                            proto_emb_src = (
                                1 - coef_src
                            ) * src_h + coef_src * sim_emb_src
                            proto_emb_dst = (
                                1 - coef_dst
                            ) * dst_h + coef_dst * sim_emb_dst
...
class AttHeteroRGCNLayer(nn.Module):
    ...
    def edge_attention(self, edges):
        src_type = edges.canonical_etype[0]
        etype = edges.canonical_etype[1]
        dst_type = edges.canonical_etype[2]
        try:
            if src_type == dst_type:
                # print(edges)
                wh2 = torch.cat(
                    [edges.src["Wh_%s" % etype], edges.dst["Wh_%s" % etype]], dim=1
                )
            else:
                if etype[:3] == "rev":
                    wh2 = torch.cat(
                    ...
                    wh2 = torch.cat(
                        [
                            edges.src["Wh_%s" % etype],
                            edges.dst["Wh_%s" % "rev_" + etype],
                        ],
                        dim=1,
                    )
        except:
    ...
    def forward(self, G, feat_dict, return_att=False):
        with G.local_scope():
            ...
            att = {}
            etypes_all = [
                i for i in G.canonical_etypes if G.edges(etype=i)[0].shape[0] != 0
            ]
            ...
            for srctype, etype, dsttype in etypes_all:
                try:
                    ...
                    print(
                        f"Destination type: {dsttype} Keys:",
                        G.nodes[dsttype].data.keys(),
                    )
                    ...
                    raise ValueError
                if return_att:
                    att[(srctype, etype, dsttype)] = (
                        G.edges[etype].data["e_%s" % etype].detach().cpu().numpy()
                    )
            ...
            return {
                ntype: G.dstdata["h"][ntype] for ntype in list(G.dstdata["h"].keys())
            }, att
...
class HeteroRGCNLayer(nn.Module):
    ...
    def gm_online(self, edges):
        etype = edges.canonical_etype[1]
        srctype = edges.canonical_etype[0]
        dsttype = edges.canonical_etype[2]
        ...
        if srctype == dsttype:
            ...
            if etype[:3] == "rev":
                gate, penalty, gate_score, penalty_not_sum = self.gate[etype][
                    self.layer
                ]([edges.src["Wh_%s" % etype], edges.dst["Wh_%s" % etype[4:]]])
            else:
                gate, penalty, gate_score, penalty_not_sum = self.gate[etype][
        ...
        if self.no_base:
            ...
            message = gate.unsqueeze(-1) * edges.src["Wh_%s" % etype] + (
                1 - gate.unsqueeze(-1)
            ) * self.baseline[etype][self.layer].unsqueeze(0)

        if self.return_gates:
            self.gate_storage[etype] = copy.deepcopy(gate.to("cpu").detach())
            self.gate_penalty_storage[etype] = copy.deepcopy(
                penalty_not_sum.to("cpu").detach()
            )
            self.gate_score_storage[etype] = copy.deepcopy(
                gate_score.to("cpu").detach()
            )
        return {"m": message}
    ...
    def graphmask_forward(self, G, feat_dict, graphmask_mode, return_gates, no_base):
        ...
        if graphmask_mode:
            self.penalty = torch.stack(self.penalty).reshape(
                -1,
            )
        ...
        return (
            {ntype: G.nodes[ntype].data["h"] for ntype in G.ntypes},
            penalty,
            self.num_masked,
        )
...
class HeteroRGCN(nn.Module):
    ...
    def forward_minibatch(
        self, pos_G, neg_G, blocks, G, mode="train", pretrain_mode=False
    ):
        ...
        scores, out_pos = self.pred(
            pos_G, G, h, pretrain_mode, mode=mode + "_pos", block=blocks[1]
        )
    ...
    def forward(
        ...
        with G.local_scope():
            ...
            if eval_pos_G is not None:
                # eval mode
                scores, out_pos = self.pred(
                    eval_pos_G, G, h, pretrain_mode, mode=mode + "_pos"
                )
                scores_neg, out_neg = self.pred(
                    neg_G, G, h, pretrain_mode, mode=mode + "_neg"
                )
                return scores, scores_neg, out_pos, out_neg
            else:
    ...
    def graphmask_forward(
        ...
        with G.local_scope():
            input_dict = {ntype: G.nodes[ntype].data["inp"] for ntype in G.ntypes}
            h_dict_l1, penalty_l1, num_masked_l1 = self.layer1.graphmask_forward(
                G, input_dict, graphmask_mode, return_gates, no_base
            )
            h_dict = {k: F.leaky_relu(h) for k, h in h_dict_l1.items()}
            h, penalty_l2, num_masked_l2 = self.layer2.graphmask_forward(
                G, h_dict, graphmask_mode, return_gates, no_base
            )

            scores_pos, out_pos = self.pred(
                pos_graph, G, h, False, mode="train_pos", only_relation=only_relation
            )
            scores_neg, out_neg = self.pred(
                neg_graph, G, h, False, mode="train_neg", only_relation=only_relation
            )
            return (
                scores_pos,
                scores_neg,
                penalty_l1 + penalty_l2,
                [num_masked_l1, num_masked_l2],
            )
    ...
    def add_graphmask_parameters(
        self,
        G,
        threshold=0.5,
        remove_key_parts=False,
        use_top_k=False,
        k=0.05,
        gate_hidden_size=32,
    ):
